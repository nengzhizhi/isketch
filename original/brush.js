			...
			if ("eyeDropper" !== f.ui.colorPicker.state) {
				...
				if (s && s.brush) {
					...
					if (!(O && s.onPointerStart && s.onPointerStart(p, ja) || V && s.onPointerMove && s.onPointerMove(p, ja) || L && s.onPointerDrag && s.onPointerDrag(p, ja))) {
						...
						if (!s.onPointer || !s.onPointer(p, x, ja)) if (O ? (e && eventjs.prevent(e), s.interval = x.interval = isFinite(p.interval) ? p.interval : a.interval || 0, s.seed = p.seed || s.seed, a.ghostDisable(), a.dragging = !0, m.load(s), 1 === p.fingers && (e = g.layer0.width, J = g.layer0.height, t.Canvas.blip(g.cache, e, J), t.Canvas.blip(g.active, e, J)), ja.cmd = "M", ja.beginPath = !0, ja.interval = x.interval, s.ghostDistance && (eventjs.supports.touch && (s.ghostX = ea, s.ghostY = Z), Q && isFinite(s.ghostX + s.ghostY) && (ja.x = s.ghostX + aa / R + W, ja.y = s.ghostY + da / R + ga, delete s.ghostX, delete s.ghostY)), U && (da = p.state + "," + ja.x + "," + ja.y + "," + r, aa = a.interval || 0, R = w.copyAttributes(a.attributes, s), R = w.clone(R, !0), c.dataChannel.send(["brush", v, da, N, K, R, aa])), !0 === s.click ? x.splice(0, 1, ja) : (x.push(ja), z.push(ja)), s.ghostDistance && (ja = {}, ja.x = ea, ja.y = Z, ja.cmd = "L", x.push(ja), z.push(ja), U && (da = "move," + ja.x + "," + ja.y + "," + r, c.dataChannel.send(["brush", v, da, N, K])))) : (e && eventjs.cancel(e), ja.cmd = L ? "L" : "Z", U && (da = p.state + "," + ja.x + "," + ja.y + "," + r, c.dataChannel.send(["brush", v, da, N, K])), s.lineRecord ? (ja.cmd = "L", x.splice(1, 1, ja), z.splice(1, 1, ja), T && s.lineRecordRotate && (r = x[0], N = x[1], ma = k.angle(r, N), r = k.atTimeOnLine(r, N, .5), N = new h.Transform, N.translate(r.x, r.y), N.rotate(-ma / 2), N.translate(-r.x, -r.y), N.flatten(x), N.flatten(z), x.__rotate = ma * t.RAD_DEG)) : s.click ? "Z" !== ja.cmd && x.splice(0, 1, ja) : (x.push(ja), z.push(ja))), s.dirtyRender = !0, s.isFromBrowser = Q, T) {
